...
def got_clicked_pos(pos, rows, width):
    gap = width // rows
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if pygame.mouse.get_pressed()[0]:
                ...
                row, col = got_clicked_pos(pos, ROWS, width)
                spot = grid[row][col]
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:
                    ...
                    algorithm(lambda: build(win, grid, ROWS, width), grid, start, end)
